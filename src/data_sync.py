...
class DatasetManager:
    ...
    def __init__(self, dataset_repo: Optional[str] = None, token: Optional[str] = None):
        ...
        # ★ 新增：最大保留历史版本数
        self.max_history_versions = int(os.environ.get('HF_MAX_HISTORY_VERSIONS', '3'))
    ...
    def prepare_backup_data(self) -> Path:
        ...
        try:
            ...
            # 环境变量（敏感信息）- 上传前 masking 处理
            if self.path_mapping['env'].exists():
                env_src = self.path_mapping['env']
                env_dst = self.temp_dir / 'config' / '.env'
                with open(env_src, 'r', encoding='utf-8') as f:
                    env_content = f.read()
                # Mask Hugging Face tokens to prevent upload rejection
    ...
    def cleanup_old_commits(self):
        """
        清理旧的提交历史，防止存储空间无限增长。
        
        当 commit 数量超过 max_history_versions 时，调用 super_squash_history
        将所有历史压缩为单个 commit。Hugging Face 会在后续自动 GC 旧 LFS 对象。
        """
        try:
            ...
            logger.info(f"Current dataset commit count: {commit_count} (limit: {self.max_history_versions})")
            
            if commit_count <= self.max_history_versions:
                logger.info("Commit count within limit, no cleanup needed")
                return True
            
            logger.warning(
                f"Commit count ({commit_count}) exceeds limit ({self.max_history_versions}), "
                f"squashing history to free storage..."
            )
            ...
            self.api.super_squash_history(
                ...
                branch="main",
                commit_message=(
                    f"Squash history (was {commit_count} commits) - "
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                )
            )
            
            logger.success("History squashed. Old LFS objects will be garbage collected.")
            return True
            
        except Exception as e:
            logger.error(f"Failed to cleanup old commits: {e}")
            return False
